...
const routes = [
  ...
  },
  {
    /*
     TODO: Remove this route
      (it's temporarily backward compatibility for moving contributions ids
      from view args to query string)
    */
    path: `${ROUTES.COMPARISON}*`,
    exact: true,
    ...
    component: ({ match, location }) => (
      <Redirect
        to={`${ROUTES.COMPARISON}?contributions=${match.params[0].split('/').join(',')}${
          location.search
            ? `&${location.search.charAt(0) === '?' ? location.search.substr(1) : location.search}`
            : ''
        }`}
      />
    ),
  ...
  },
  {
    ...
    // eslint-disable-next-line react/prop-types
    component: ({ match }) => (
      <Redirect to={`/resource/${match.params.resourceId}/${descriptionSection}`} />
    ),
  },
