...
export type OntologyClass = {
    ...
    suggestedProperty?: boolean;
    color?: string;
};

export const SURVEY_TABLES_IRI = 'SurveyTables';
export const CSVW_TABLE_IRI = 'RegularTable';
...
const useOntology = () => {
    ...
    const findByType = (type: string) => classes.find((_class) => _class.iri === type);
    const findByLabel = (label: string) => classes.find((_class) => _class.label === label);
