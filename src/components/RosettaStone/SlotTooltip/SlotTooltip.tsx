            ...
            content={
                <>
                    <HeaderStyled className="pb-2 mb-2 flex">
                        <div className="grow">{slot.placeholder}</div>
                    </HeaderStyled>

                    {slot.description && (
                        <div>
                            <b>Description:</b>
                            <p className="text-sm mb-0">{slot.description}</p>
                        </div>
                    )}
                    ...
                    {'datatype' in slot && slot.datatype?.id && (
                        <>
                            <div>
                                <b>Range:</b>
                                <div>
                                    <p className="text-sm mb-0">{slot.datatype.label}</p>
                                </div>
                            </div>
                            {'pattern' in slot && slot.pattern && (
                                <div>
                                    <b>Pattern:</b>
                                    <p className="text-sm mb-0">{slot.pattern}</p>
                                </div>
                            )}
                            {'min_inclusive' in slot && slot.min_inclusive && (
                                <div>
                                    <b>Min inclusive:</b>
                                    <p className="text-sm mb-0">{slot.min_inclusive}</p>
                                </div>
                            )}
                            {'max_inclusive' in slot && slot.max_inclusive && (
                                <div>
                                    <b>Max inclusive:</b>
                                    <p className="text-sm mb-0">{slot.max_inclusive}</p>
                                </div>
                            )}
                        </>
                    )}
                    {slot.min_count !== undefined && slot.min_count !== null && (
                        <div>
                            <b>Min count:</b>
                            <p className="text-sm mb-0">{slot.min_count}</p>
                        </div>
                    )}
                    {slot.max_count !== undefined && slot.max_count !== null && (
                        <div>
                            <b>Max count:</b>
                            <p className="text-sm mb-0">{slot.max_count}</p>
                        </div>
                    )}
                </>
            }
