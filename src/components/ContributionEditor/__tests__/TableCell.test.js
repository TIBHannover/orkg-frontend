...
describe('TableCell', () => {
    it('should render when no values are provided', () => {
        render(<TableCell values={[]} contributionId="R1" propertyId="P1" />);
    });

    it('should render a resource when provided as value', () => {
        const { label } = setup();
        expect(screen.getByRole('button', { name: label })).toBeInTheDocument();
    });

    it('should render a literal when provided as value', () => {
        const { label } = setup();

        expect(screen.getByText(label)).toBeInTheDocument();
    });

    it('should render multiple cell values with different types', () => {
