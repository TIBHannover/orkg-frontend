...
const PrintView: FC<PrintViewProps> = ({ comparisonId }) => {
    ...
    return (
        <Alert color="info" fade={false} className="hidden d-print-block">
            ...
            <table className="table table-sm mb-2">
                <tbody>
                    ...
                    {comparison?.authors && comparison.authors.length > 0 && (
                        <tr>
                            <th className="font-semibold">{pluralize('Author', comparison.authors.length)}</th>
                            <td>{comparison.authors.map((author) => author.name).join(', ')}</td>
                        </tr>
                    )}
                    {comparisonContents?.titles && (
                        <tr>
                            <th className="font-semibold">Number of contributions</th>
                    ...
                    {comparison?.identifiers.doi && comparison.identifiers.doi.length > 0 && (
                        <tr>
                            <th className="font-semibold">DOI</th>
                            <td>
