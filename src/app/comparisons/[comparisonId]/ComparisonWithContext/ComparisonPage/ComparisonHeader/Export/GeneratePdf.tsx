...
const GeneratePdf: FC<GeneratePdfProps> = ({ id }) => {
    ...
    const handleExport = async () => {
        setIsLoading(true);
        const header = document.getElementById(id)?.querySelectorAll('thead > tr')[0] as HTMLElement;
        const headerHeightMm = header?.offsetHeight;
        const headerWidthMm = header?.offsetWidth;
        const allRows = Array.from((document.getElementById(id)?.querySelectorAll('tbody > tr') as NodeListOf<HTMLElement>) ?? []);
