...
const useQualityReport = () => {
    ...
    const performQualityEvaluation = useCallback(async () => {
        try {
            ...
            // suggestions
            const allValues = table.flatMap((row) => row.values.filter((value) => value));
            const propertiesWithoutDescription = properties.filter((property) => property.description == null || property.description === '');

            const resources = allValues.filter((entity) => entity?._class === 'resource_ref');
            const literals = allValues.filter((entity) => entity?._class === 'literal_ref');
            const sameAsStatements = await Promise.all(
                resources.map((resource) => getStatements({ subjectId: resource.id, predicateId: PREDICATES.SAME_AS })),
            );
            const qualityCriteria = [
                        ...
                        return {
                            ...
                            evaluation: passing ? (
                                ...
                                <>
                                    ...
                                    <ul>
                                        {propertiesWithoutDescription.map((property: ComparisonPath, index: number) => (
                                            <li key={index}>
                ...
                },
                {
                    ...
                    performEvaluation: () => {
                        const passing = sameAsStatements.flat().length > 0;
                        return {
                            passing,
                            evaluation: passing
